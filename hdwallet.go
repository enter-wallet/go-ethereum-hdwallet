...
func GenAddrFromPub(pub, soft_path string) (string, error) {
	...
	parsed, err := ParseRelatedDerivationPath(soft_path)
	if err != nil {
		panic(err)
	}
	fmt.Println(parsed.String())
	account, err := wallet.DeriveFromPath(parsed, false)
	if err != nil {
		return "", err
	}
...
func ParseRelatedDerivationPath(path string) (accounts.DerivationPath, error) {
	var result accounts.DerivationPath
